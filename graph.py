...
plt.plot_date(pd.to_datetime(c_flood.index), c_flood['C_Stage'], fmt='rx', xdate=True)

# Site D
plt.plot_date(pd.to_datetime(data.index), data['D_Stage'], fmt='m-', xdate=True)
d_flood = data.loc[data['D_Flood'] == 1]
plt.plot_date(pd.to_datetime(d_flood.index), d_flood['D_Stage'], fmt='r^', xdate=True)

# Site E
plt.plot_date(pd.to_datetime(data.index), data['E_Stage'], fmt='c-', xdate=True)
e_flood = data.loc[data['E_Flood'] == 1]
plt.plot_date(pd.to_datetime(e_flood.index), e_flood['E_Stage'], fmt='rv', xdate=True)

# Labeling
plt.xlabel('Date')
plt.ylabel('Stage (ft)')
plt.legend(['A_Stage', 'A_Flood', 'B_Stage', 'B_Flood', 'C_Stage', 'C_Flood',
            'D_Stage', 'D_Flood', 'E_Stage', 'E_Flood'])
...
# Plot flow data from one Crabtree and one urban site with flood markers
# Site B
plt.plot_date(pd.to_datetime(data.index), data['B_Flow'], fmt='b-', xdate=True)
b_flood = data.loc[data['B_Flood'] == 1]
plt.plot_date(pd.to_datetime(b_flood.index), b_flood['B_Flow'], fmt='ro', xdate=True)

# Site X
plt.plot_date(pd.to_datetime(data.index), data['X_Flow'], fmt='g-', xdate=True)
x_flood = data.loc[data['X_Flood'] == 1]
plt.plot_date(pd.to_datetime(x_flood.index), x_flood['X_Flow'], fmt='r*', xdate=True)

# Labeling
plt.xlabel('Date')
plt.ylabel('Flow Volume (cfs)')
plt.legend(['B_Flow', 'B_Flood', 'X_Flow', 'X_Flood'])
plt.title('Crabtree and Urban Site Flood Volume with Flood Markers')
plt.show()


# Plot flow data from one Crabtree and one urban site with flood markers
# Site D
plt.plot_date(pd.to_datetime(data.index), data['D_Precip'], fmt='b-', xdate=True)
d_flood = data.loc[data['D_Flood'] == 1]
plt.plot_date(pd.to_datetime(d_flood.index), d_flood['D_Precip'], fmt='ro', xdate=True)

# Site Y
plt.plot_date(pd.to_datetime(data.index), data['Y_Precip'], fmt='g-', xdate=True)
y_flood = data.loc[data['Y_Flood'] == 1]
plt.plot_date(pd.to_datetime(y_flood.index), y_flood['Y_Precip'], fmt='r*', xdate=True)

# Labeling
plt.xlabel('Date')
plt.ylabel('Precipitation (in)')
plt.legend(['D_Precip', 'D_Flood', 'Y_Precip', 'Y_Flood'])
plt.title('Crabtree and Urban Site Precipitation Levels with Flood Markers')
plt.show()
